...
def main():
    ...
    with open('data/train.csv', 'w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(comments_clean)
    ...
    client = MlflowClient()
    last_version_clf_model = get_version_model(config['model_name'][config['clf']], client)
    last_version_vec = get_version_model(config['model_vec'], client)

    yaml_file = yaml.safe_load(open(config_path))
    yaml_file['predict'][f'version_{config["model_name"][config["clf"]]}'] = int(last_version_clf_model)
